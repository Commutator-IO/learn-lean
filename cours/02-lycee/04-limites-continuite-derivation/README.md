...
Fichier : `LimitesContinuiteDerivation.lean`
...
| Opérations sur les limites ; limite d'une composée | TS | oui | `operations_sur_les_limites_limite_d_une_composee` | ☐ |
| Croissances comparées : `eˣ/xⁿ → +∞`, `ln x / x → 0`, `x ln x → 0` | TS |  | `croissances_comparees` | ☐ |
| Continuité en un point, sur un intervalle ; toute fonction dérivable est continue | TS | oui | `continuite_en_un_point_sur_un_intervalle_toute` | ☐ |
| Théorème des valeurs intermédiaires | TS | oui | `theoreme_des_valeurs_intermediaires` | ☐ |
...
| Opérations : `(u + v)'`, `(uv)'`, `(1/v)'`, `(u/v)'` | 1S |  | `operations` | ☐ |
| Dérivée d'une composée `(v ∘ u)' = u' × (v' ∘ u)` ; cas `u(ax + b)` | 1S / TS |  | `derivee_d_une_composee_cas` | ☐ |
| Signe de `f'` et sens de variation de `f` | 1S | oui | `signe_de_et_sens_de_variation_de` | ☐ |
