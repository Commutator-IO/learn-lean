...
\begin{figure}[h]
\centering
\input{figures/distance_minimale_au_pied}
\caption{Le pied de la perpendiculaire est le point de la droite le plus proche de $p$.}
\end{figure}
...
\begin{proof}
Mathlib ne définit pas la médiatrice comme l'ensemble des points équidistants, mais comme la
droite passant par le milieu de $[ab]$ et orthogonale à $\vec{ab}$. L'énoncé est donc
l'équivalence entre les deux descriptions : celle du collège et celle de la bibliothèque.

Elle tient à une seule identité du produit scalaire. En posant $u = \vec{ma}$ et
$v = \vec{mb}$,
\[ \langle u + v,\ u - v \rangle = \|u\|^2 - \|v\|^2 , \]
de sorte que $\|u\| = \|v\|$ équivaut à l'orthogonalité de $u + v$ et $u - v$. Or $u - v$
est le vecteur $\vec{ba}$, et $u + v$ est le double du vecteur qui joint $m$ au milieu de
$[ab]$ : cette orthogonalité est exactement l'appartenance à la médiatrice.

Reste l'ordre des arguments de la distance, que la symétrie $d(m, a) = d(a, m)$ échange.
\end{proof}
...
\begin{proof}
La démonstration ramène d'abord l'énoncé aux vecteurs. En posant $x = \vec{ba}$ et
$y = \vec{bc}$, les trois angles du triangle sont les angles de vecteurs $(x, y)$,
$(x, x - y)$ et $(y, y - x)$, et il s'agit de voir que leur somme vaut $\pi$.

Le point d'appui est l'additivité de l'angle non orienté : lorsqu'un vecteur appartient au
secteur délimité par deux autres, l'angle des deux extrêmes est la somme des deux angles
partiels. Appliquée au vecteur $x - y$, qui pointe de $c$ vers $a$, elle découpe l'angle
plat en les trois angles du triangle ; l'angle entre $y$ et $-y$ valant $\pi$, la somme
suit.

C'est un tout autre chemin que celui du collège, qui trace la parallèle à $(bc)$ passant par
$a$ et lit les angles alternes-internes. Les deux disent la même chose ; le second demande
le postulat des parallèles, le premier le produit scalaire.

L'hypothèse $b \ne a$ est nécessaire : entre deux points confondus, l'angle n'est pas
défini.
\end{proof}
...
\begin{proof}
Un angle non orienté est défini dans Mathlib par
\[ \widehat{(u, v)} = \arccos \frac{\langle u, v\rangle}{\|u\|\,\|v\|} , \]
et $\arccos$ est injective sur $[-1, 1]$ : l'égalité des deux angles donne donc l'égalité
des deux quotients.

En posant $x = \vec{ab}$ et $y = \vec{ac}$, l'hypothèse s'écrit
\[ \frac{\langle x,\ x - y\rangle}{\|x\|\,\|x-y\|}
   = \frac{\langle y,\ y - x\rangle}{\|y\|\,\|y-x\|} . \]
Les deux dénominateurs partagent le facteur $\|x - y\|$, qu'on simplifie — il n'est nul que
si $b = c$, cas où il n'y a rien à démontrer. En développant les deux produits scalaires et
en réduisant, on aboutit à une alternative : ou bien $\|x\| = \|y\|$, ce qui est la
conclusion cherchée, ou bien
\[ \frac{\langle x, y\rangle}{\|x\|\,\|y\|} = -1 , \]
c'est-à-dire l'angle en $a$ est plat.

C'est ce second cas que l'hypothèse écarte, et il faut l'écarter : trois points alignés
donnent bien deux angles égaux sans que le triangle soit isocèle — il n'y a alors pas de
triangle du tout.
\end{proof}
...
\caption{Le triangle rectangle en $b$ : le carré de l'hypoténuse $[ac]$ est la somme des carrés des deux autres côtés.}
\end{figure}
...
\begin{figure}[h]
\centering
\input{figures/theoreme_des_milieux}
\caption{Le segment qui joint les milieux de deux côtés est parallèle au troisième et en mesure la moitié.}
\end{figure}
...
\begin{figure}[h]
\centering
\input{figures/thales_rapports}
\caption{Deux droites sécantes en $a$, coupées par deux parallèles : les longueurs sont multipliées par le même rapport $t$.}
\end{figure}
...
\begin{proof}
Mathlib ne construit pas le centre du cercle circonscrit en coupant deux médiatrices, comme
le fait le collège, mais pour un simplexe de dimension quelconque — ici un triangle non
aplati.

L'idée reste la même. Chercher un point équidistant des trois sommets, c'est chercher un
point appartenant aux trois médiatrices ; chacune est un sous-espace affine, et
l'indépendance affine des sommets garantit que leur intersection avec le plan du triangle se
réduit à un point. Ce point est le \emph{circoncentre}, sa distance commune aux sommets est
le \emph{circonrayon}, et c'est cette égalité, sommet par sommet, que la bibliothèque
établit.

La démonstration ci-dessus ne fait plus que l'appliquer aux trois sommets, en échangeant
l'ordre des arguments de la distance. L'hypothèse d'indépendance affine traduit « les trois
points ne sont pas alignés » : sans elle les médiatrices sont parallèles, et il n'y a pas de
cercle circonscrit.
\end{proof}
...
\begin{proof}
Notons $o$ le centre du cercle et $r$ son rayon. Comme $[ac]$ est un diamètre, $o$ est le
milieu de $[ac]$. L'angle en $b$ est droit si et seulement si le produit scalaire
$\langle \vec{ba},\ \vec{bc}\rangle$ est nul.

On décompose les deux vecteurs en passant par le centre :
\[ \vec{ba} = \vec{bo} + \vec{oa} , \qquad \vec{bc} = \vec{bo} + \vec{oc} . \]
Le point $o$ étant le milieu de $[ac]$, on a $\vec{oa} = -\vec{oc}$. En développant, les
deux termes croisés s'annulent et il reste
\[ \langle \vec{ba},\ \vec{bc}\rangle = \|\vec{bo}\|^2 - \|\vec{oa}\|^2 = ob^2 - r^2 . \]

Le produit scalaire est donc nul exactement quand $ob = r$, c'est-à-dire quand $b$ est sur
le cercle. Un seul calcul donne les deux sens de l'équivalence : le triangle inscrit dans un
demi-cercle est rectangle, et l'hypoténuse d'un triangle rectangle est un diamètre de son
cercle circonscrit.
\end{proof}
...
\begin{figure}[h]
\centering
\input{figures/triangle_inscrit_demi_cercle}
\caption{Un triangle dont un côté est un diamètre est rectangle au troisième sommet.}
\end{figure}
...
\begin{proof}
Mathlib ne définit pas l'orthocentre comme l'intersection des hauteurs — ce serait supposer
ce qu'on veut démontrer. Il le définit comme le \emph{point de Monge} du triangle, une
construction qui vaut en toute dimension.

Et ce point de Monge n'est pas davantage défini par une intersection : il est posé sur la
droite d'Euler, à une position fixée à partir du centre du cercle circonscrit $O$ et du
centre de gravité $G$ — ces trois points sont alignés dans cet ordre, avec
$OG : GM = (n-1) : 2$. La bibliothèque démontre \emph{ensuite} que le point ainsi construit
appartient à chacun des plans de Monge, et qu'il est leur unique point commun. En dimension
$2$, ces plans sont les hauteurs.

Le concours en découle : un même point appartient aux trois hauteurs. C'est l'inverse de la
démarche du collège, qui montre d'abord que deux hauteurs se coupent, puis que la troisième
passe par leur point d'intersection.
\end{proof}
...
\begin{proof}
Mathlib construit le centre du cercle inscrit d'un simplexe et son rayon, puis démontre que
la distance de ce centre à chacune des faces vaut ce rayon. Pour un triangle, les faces sont
les côtés, et le point de contact est le pied de la perpendiculaire abaissée du centre.

Le concours des bissectrices s'en déduit par la caractérisation qu'en donne le collège :
être à égale distance de deux côtés, c'est être sur leur bissectrice intérieure. Un point à
égale distance des trois côtés est donc sur les trois bissectrices, qui se coupent en ce
point.

L'énoncé formalisé porte sur la distance, non sur les bissectrices : c'est la distance que
la bibliothèque sait nommer, et l'équivalence entre les deux formulations reste ici du côté
du texte.
\end{proof}
