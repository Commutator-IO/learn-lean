% Fichier engendré par tools/generate-tex.py à partir de 07-nombres-complexes.
...
\begin{proof}
Les deux premiers points se lisent sur la construction de $\mathbb{C}$ comme couples de
réels : $z$ y \emph{est} le couple $(\mathrm{Re}(z), \mathrm{Im}(z))$, et l'addition comme
la multiplication sont posées coordonnée par coordonnée, ce qui donne à la fois
$z = \mathrm{Re}(z) + i\,\mathrm{Im}(z)$ et $i^{2} = -1$.
...
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L20}{NombresComplexes.lean\#L20}
...
algébrique}, un réel. Or le module est précisément défini comme la racine
carrée de cette norme, de sorte que $\mathrm{N}(z) = |z|^{2}$ ; il reste à voir l'égalité de
réels comme une égalité de complexes.
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L30}{NombresComplexes.lean\#L30}

\begin{figure}[h]
\centering
\input{figures/conjugue_proprietes}
\caption{Un complexe et son conjugué sont symétriques par rapport à l'axe des réels. Un complexe égal à son conjugué est donc sur cet axe : c'est un réel.}
\end{figure}
...
vérifiée.
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L41}{NombresComplexes.lean\#L41}
...
\begin{proof}
Les deux propriétés viennent de ce que $\mathbb{C}$ est un corps normé dans la bibliothèque,
et que le module en est la norme. Voici ce qui les démontre.

Pour la multiplicativité, on passe par le carré et le conjugué :
\[ |zz'|^2 = zz'\overline{zz'} = z\bar z\, z'\overline{z'} = |z|^2 |z'|^2 , \]
et les deux membres étant positifs, on prend la racine.

L'inégalité triangulaire se ramène, en élevant au carré, à
$\mathrm{Re}(z\overline{z'}) \le |z|\,|z'|$, qui est l'inégalité de Cauchy--Schwarz dans le
plan : la partie réelle d'un produit ne dépasse pas le produit des modules.
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L58}{NombresComplexes.lean\#L58}
...
\begin{proof}
L'argument n'est pas posé par cette égalité dans la bibliothèque : il y est \emph{construit},
puis l'égalité est démontrée.

La construction se fait par cas sur la position de $z$ dans le plan :
\[ \arg z = \begin{cases}
   \arcsin\bigl(\mathrm{Im}(z)/|z|\bigr) & \text{si } \mathrm{Re}(z) \ge 0 ,\\
   \arcsin\bigl(-\mathrm{Im}(z)/|z|\bigr) + \pi & \text{si } \mathrm{Re}(z) < 0
     \text{ et } \mathrm{Im}(z) \ge 0 ,\\
   \arcsin\bigl(-\mathrm{Im}(z)/|z|\bigr) - \pi & \text{sinon} ,
   \end{cases} \]
les deux derniers cas ramenant dans $]-\pi\,;\pi]$ l'angle du demi-plan gauche, où
l'$\arcsin$ seul ne suffirait pas à distinguer un point de son symétrique.

La bibliothèque démontre ensuite, dans chacun de ces cas, que
$\cos(\arg z) = \mathrm{Re}(z)/|z|$ et $\sin(\arg z) = \mathrm{Im}(z)/|z|$, ce qui donne
l'égalité voulue en multipliant par $|z|$. Le résultat y est marqué comme simplification
automatique, si bien que la transcription formelle se réduit à l'invoquer.
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L65}{NombresComplexes.lean\#L65}

\begin{figure}[h]
\centering
\input{figures/forme_trigonometrique}
\caption{Le point d'affixe $z$ : son module est la longueur $OM$, son argument l'angle que $[OM)$ fait avec l'axe des réels. Les projections donnent la forme algébrique, l'angle et la longueur la forme trigonométrique.}
\end{figure}
...
\begin{proof}
L'énoncé mérite qu'on s'y arrête, parce que sa forme Lean est exactement celle du cours et
pas celle qu'on écrirait naïvement.

L'argument est un réel choisi dans $]-\pi\,;\pi]$. Avec ce choix,
...
\og{} $[2\pi]$ \fg{} du lycée.
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L71}{NombresComplexes.lean\#L71}
...
\begin{proof}
On regroupe les termes pairs et impairs de la série de l'exponentielle complexe : les
premiers donnent la série du cosinus, les seconds celle du sinus, et le facteur $i$ sépare
les deux.
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L78}{NombresComplexes.lean\#L78}
...
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L83}{NombresComplexes.lean\#L83}
...
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L102}{NombresComplexes.lean\#L102}
...
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L113}{NombresComplexes.lean\#L113}
...
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L147}{NombresComplexes.lean\#L147}
...
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L151}{NombresComplexes.lean\#L151}
...
l'est.
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L167}{NombresComplexes.lean\#L167}
...
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L192}{NombresComplexes.lean\#L192}
...
$|e^{i\theta}|\,|z - z'|$. Or $|e^{i\theta}|^{2} = e^{i\theta}\,\overline{e^{i\theta}}
= e^{i\theta}e^{-i\theta} = 1$ pour $\theta$ réel, d'où la conservation des distances.
...
\end{proof}
\source{https://github.com/Commutator-IO/learn-lean/blob/main/courses/02-lycee/07-nombres-complexes/NombresComplexes.lean\#L210}{NombresComplexes.lean\#L210}
