...
\begin{proof}
Pour $A > 0$ donné, tout $x$ tel que $0 < x < 1/A$ vérifie $1/x > A$ : l'inverse dépasse
donc toute borne dès qu'on s'approche assez de zéro par la droite. Comme $1/x$ et $x^{-1}$ sont deux écritures de la même
...
\begin{proof}
Le programme du lycée admet ce résultat ; en voici l'argument.

Si $|f(x) - \ell| < \varepsilon/2$ et
...
\begin{proof}
Tout repose sur la minoration de l'exponentielle par un terme de sa série : pour $x \ge 0$,
...
\begin{proof}
La bibliothèque énonce que $\ln$ est \emph{négligeable} devant la fonction identité en
$+\infty$, ce qui signifie précisément que le quotient des deux tend vers zéro ; elle le
démontre dans le cas général de $(\ln x)^n$.

Voici l'argument, dans le cas qui nous occupe. La bibliothèque établit d'abord la
majoration
\[ \ln t \le t - 1 \qquad \text{pour tout } t > 0 , \]
qui exprime que la courbe du logarithme reste sous sa tangente en $1$. Appliquée à
$t = \sqrt x$ :
\[ \tfrac12 \ln x = \ln \sqrt x \le \sqrt x - 1 \le \sqrt x , \]
d'où, pour $x > 0$,
\[ 0 \le \frac{\ln x}{x} \le \frac{2\sqrt x}{x} = \frac{2}{\sqrt x} . \]
Le majorant tend vers zéro, donc le quotient aussi.

Le détour par $\sqrt x$ est ce qui fait tout : appliquée directement à $x$, la majoration
$\ln x \le x - 1$ ne donne qu'un quotient majoré par $1$.
\end{proof}
...
\begin{figure}[h]
\centering
\input{figures/theoreme_des_valeurs_intermediaires}
\caption{Une fonction continue prend toute valeur $k$ comprise entre $f(a)$ et $f(b)$.}
\end{figure}
...
\begin{figure}[h]
\centering
\input{figures/equation_de_la_tangente}
\caption{La tangente au point d'abscisse $a$ : elle passe par $(a, f(a))$ et sa pente est le nombre dérivé.}
\end{figure}
...
\begin{proof}
Les sept dérivées sont celles du programme ; deux d'entre elles demandent une mise en
forme avant d'y ressembler.
...
\begin{proof}
La somme, le produit et le quotient sont les trois règles usuelles, qui s'établissent en
écrivant la dérivée comme limite du taux d'accroissement et en développant.
...
\begin{proof}
La bibliothèque démontre la règle de dérivation en chaîne sous sa forme générale, celle des
différentielles : dériver, c'est approcher la fonction par une application linéaire au
voisinage du point, et composer deux fonctions revient à composer leurs approximations.

Écrivons-la à une variable. Au voisinage de $x$,
\[ u(x + h) = u(x) + u'h + o(h) , \]
et au voisinage de $u(x)$,
\[ v\bigl(u(x) + k\bigr) = v\bigl(u(x)\bigr) + v'k + o(k) . \]
En posant $k = u'h + o(h)$, qui tend vers $0$ avec $h$ :
\[ v\bigl(u(x + h)\bigr) = v\bigl(u(x)\bigr) + v'u'h + o(h) , \]
ce qui est exactement dire que $v \circ u$ est dérivable en $x$, de nombre dérivé $v'u'$.

Le produit se lit ainsi : $u'$ mesure la vitesse à laquelle $u$ parcourt son image, et $v'$
la sensibilité de $v$ au point atteint.
\end{proof}
...
\begin{proof}
Une dérivée seconde positive rend $f'$ croissante, et une fonction dont la dérivée croît
est convexe \textemdash{} sa courbe reste au-dessous de ses cordes.

Les hypothèses du fichier portent sur le segment fermé tout entier, là où l'énoncé général
se contente de son intérieur : c'est plus fort qu'il ne faut, et sans conséquence ici.
\end{proof}
